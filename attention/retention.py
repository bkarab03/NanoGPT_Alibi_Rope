...
from attention.util import ComplexGroupNorm
...
class SimpleRetention(nn.Module):
    def __init__(self, hidden_size, gamma, precision="single"):
        ...
        self.complex_type = torch.float32
    ...
    def forward(self, X):
        ...
        def forward(self, X):
            ...
            Q = (X @ self.W_Q.to(self.complex_type))
            K = (X @ self.W_K.to(self.complex_type))
...
class MultiScaleRetention(nn.Module):
    def __init__(self, config):
        ...
        self.gammas = (1 - torch.exp(torch.linspace(math.log(1/32), math.log(1/512), heads, dtype=self.real_type))).detach().tolist()
